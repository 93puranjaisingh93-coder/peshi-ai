        ...
        with st.container(border=True):
            ...
            if st.button("Execute Core Strategy Brief", use_container_width=True):
                with st.spinner("Synthesizing legal framework..."):
                    try:
                        client = genai.Client(api_key=st.session_state.stored_api_key)
                        contents = ["Generate a high-level case strategy brief. Identify the core dispute, point out any statutory timelines, and cite page numbers for evidence."] + st.session_state.gemini_files
                        response = client.models.generate_content(
                            model='gemini-2.5-flash',
                            contents=contents,
                            config=types.GenerateContentConfig(system_instruction=PESHI_SYSTEM_INSTRUCTION)
